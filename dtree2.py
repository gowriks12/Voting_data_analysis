...
class dtree2:
    ...
    def read_data(self, filename):
        ...
        # test_i = [x for x in data if x not in train]
        test_i = data[train_size:]
        test = []
        for i in range(len(train)):
            train_class.append(train[i][0])
            train[i] = train[i][1:]
        for i in range(len(test_i)):
            test_class.append(test_i[i][0])
            test.append(test_i[i][1:])

        self.classes = train_class
        # print("################ {}".format(test_class))
        return train, test, train_class, test_class, self.featureNames
    ...
    def make_tree(self, data, classes, featureNames, maxlevel=-1, level=0, forest=0):
        ...
        if nData == 0 or nFeatures == 0 or (0 <= maxlevel < level):
            ...
            ginis = np.zeros(nFeatures)
            featureSet = range(nFeatures)
            if forest != 0:
                np.random.shuffle(featureSet)
                featureSet = featureSet[0:forest]
            for feature in featureSet:
                ginis[feature] = self.gini_index(data, classes, feature)
                # print("#####", feature,"*****", g)
                # print(ginis[feature])

            bestFeature = np.argmin(ginis)
            # print(bestFeature)
    ...
    def gini_index(self, data, classes, feature):
        nData = len(data)
        giniIndex=0
        ...
        for value in values:
            ...
            for classValue in classValues:
                ...
                classIndex += 1
            for classIndex in range(len(classValues)):
                gini[valueIndex] += self.calc_gini(float(classCounts[classIndex]) / np.sum(classCounts))
            gini[valueIndex] = 1 - gini[valueIndex]
            giniIndex = giniIndex + float(featureCounts[valueIndex]) / nData * gini[valueIndex]
            valueIndex += 1
        return giniIndex
